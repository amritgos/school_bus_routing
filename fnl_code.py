...
def VRP (dist_mat, time_mat, cust_stop_mat, bs_points, cp_points, w_dist_lim, cap_lim, dist_lim, tim_lim):
    ...
    # print(necessary_bs & (~remove_bs_pts))
    ...
    for iter_no in range(5):
        ...
        for k in unique_labels:
            ...
            #else:
                #curr_potential_bs = np.vstack((curr_potential_bs, curr_bs_pts))
                #for st in class_ind:
                #    curr_alloc_cp.update(busstop_customer_alloc[st])
    ...
    for i in range(len(list_ind)-1,-1,-1):
        for j in range(i-1,-1,-1):
            if list(bus_coord[list_ind[i]]) == list(bus_coord[list_ind[j]]):
                del list_ind[i]
                break
    #list_ind = [0, 1, 2, 5, 6, 8, 17, 20, 27, 32, 39, 41, 42, 50, 54, 59, 62]
#     list_ind = [i for i in range(dist_mat.shape[0])]
    ...
    for i in range(5):
        ...
        while (len(list_stop_pairs) >=0 and (prob_sum >= 0.1)):
            ...
            #print(prob_sum)
            #print (len(list_stop_pairs))
            #if len(list_stop_pairs) ==1:
            #    print(prob)
    ...
    for i in range(5):
        ...
        print (l)
        l_opt = l
        #arr = [rem_ins_w_rt,rem_ins_bwt_rt,replace_nodes,remove_node]
        K = 0
        cnt = 0
        flag = 0
        array = route.copy()
...
#             continue
        ...
        if (check == 1): 
            ...
            if cnt > 0:
                ...
                check = -1
#     z = 0
#     cust_alloc_vnd = np.asarray(cust_alloc_vnd)
#     curr_route_cust = (cust_alloc_vnd[:, z] == 1)
#     bus_no = []
#     for i in range(1,len(best_sol)):
#         if best_sol[i] != 0:
#             #coord.append(list(bus_coord[list_ind[best_sol[i]]]))
#             bus_no.append(best_sol[i])
#             #print (coord)
#         else:
#             curr_route_cust = (cust_alloc_vnd[:, z] == 1)
#             curr_route_cust  = np.asarray(curr_route_cust)
#             dist_mat_rc = cust_stop_mat[curr_route_cust]
#             dist_mat_rc = dist_mat_rc[:,bus_no]
#             alloc_stop_ind = np.argmin(dist_mat_rc, axis = 1)
#             alloc_stop_ind = np.reshape([len(cust_stop_mat)])
#             alloc_stop = np.zeros([len(cust_stop_mat)])
#             alloc_stop = bus_no[alloc_stop_ind]
#             z += 1
#             bus_no = []
#     x = list(range(len(it_dist)))
#     plt.plot(x, it_dist)
#     plt.show()
    print (l_opt, route,'\n', best_sol)
    #obj_val, cust_alloc_vnd = check_feasible_vnd(list(map(int,best_sol)), cust_stop_mat, w_dist_lim, cap_lim, l)
...
########### MAIN ################
#dist_mat, time_mat, cust_stop_mat, bs_points, cp_points, w_dist_lim, cap_lim, dist_lim, time_lim
